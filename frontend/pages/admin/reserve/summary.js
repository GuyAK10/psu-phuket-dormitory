...
const summary = () => {
    ...
    const getUniversityYear = async () => {
        ...
        console.log(data)
        setUniversityYear(data.data)     
    }
    const getFloor = async () => {
        
        if (semester!=="") {
            
            const floor = await get(`staff/room/${floorList[current]}/${semester}`)
    ...
    return (
        <div className="reserve-summary">
            <div className="reserve-select h-screen">
            ...
            {semester!=="" &&floorDetail.length!==0?
                ...
                :semester!==""&&floorDetail.length==0? <div>ไม่มีข้อมูลในระบบ</div>:""}
